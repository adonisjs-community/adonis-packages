...
<template>
  <Listbox v-model="model" :multiple="multiple" as="div">
    <div class="relative">
      <ListboxButton
        v-slot="{ open }"
        class="relative text-sm w-full cursor-pointer py-2 pl-5 pr-10 text-left text-base12 shadow-sm md:w-[230px] sm:leading-6"
        :class="{ '!bg-base3 rounded border border-base5': !group }"
      >
        ...
        <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center pr-2">
          <i
            class="i-tabler:chevron-down transform text-sm text-base-11 transition-transform duration-200"
      ...
      >
        <ListboxOptions
          class="absolute z-10 mt-2 w-full overflow-auto rounded bg-base3 border border-base4 px-2 py-2 text-sm shadow-lg"
        >
          <ListboxOption
          ...
          >
            <li
              ...
              :class="[active ? 'bg-base4' : '']"
            >
              <div class="flex flex-col font-content">
                <span :class="{ 'text-sm': !option.description, 'text-base': option.description }">
                  {{ option.label }}
                </span>
                <span
                  v-if="option.description"
                  class="pr-8 text-xs font-thin"
                  :class="[active ? 'text-base11' : 'text-base10']"
                >
                  {{ option.description }}
                </span>
              </div>
              ...
              <span
                ...
                :class="[active ? 'text-base12' : 'text-base10']"
              >
                <i class="i-tabler:check text-sm text-base-11" />
              </span>
            </li>
          </ListboxOption>
        </ListboxOptions>
